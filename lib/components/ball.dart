...
import 'package:ecoslam_game/constants/image_slam.dart';
...
class BallComponent extends BodyComponent<BasketBallGame>
  ...
  BallComponent(this.gameSize1,this.valor) : super(priority: 7);
...
..sprite = await gameRef.loadSprite(ImageSlam.getRandomBasketballSprite())
      
      ..opacity = 1
      ..size = Vector2.all(3.3);
    add(_ball);
    renderBody = false;
    body.sleepTime = 5;
      final String randomImageName = ImageSlam.getRandomBasketballSprite();

           tipoDeImagen = ImageSlam.getValueForImage(randomImageName);
  ...
  if (_hasPeaked &&
    isBallBetweenRim(body.position, gameSize1) &&
    !_hasRecordedScore &&
    _hasReachedTop ) {
  ...
  _hasRecordedScore = true;
    ...
    if (body.position.y <= 55) {
      ...
      //if the ball no longer has velocity to move upwards, create new ball
        ...
        _hasPeaked = true;
        _createNewBall ? gameRef.add(BallComponent(gameSize1,valor)) : null;
...
//to check if the ball entered the rim
 bool isBallBetweenRim(Vector2 ball, Vector2 gameRef) {


Vector2 startRim = Vector2((gameRef.x), gameRef.y-2);
  //print("inicio$tipoDeImagen");
Vector2 endRim = Vector2((gameRef.x)+9.1, gameRef.y-2);
  //print("fin$endRim");
 //   print("ball$ball");
  double ballToStartRim = (ball - startRim).length;
 // print("ballToStartRim$ballToStartRim");
  double ballToEndRim = (ball - endRim).length;
  //  print("ballToEndRim$ballToEndRim");
  double startRimToEndRim = (startRim - endRim).length;
   //   print("startRimToEndRim$startRimToEndRim");
  // Cambia este valor para ajustar la tolerancia entre la pelota y el aro
